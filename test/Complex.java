...
public class Complex implements Comparable{
  ...
  */
  public String toString(){
      return(String.format("%.5f",a)+"+"+String.format("%.5f",b)+"i");
  }
  ...
  */
  public double arg(){
    ...
    if (a>0) return(theta);
    else if (a<0.0 && b>=0.0) {return(theta+Math.PI);}
    else if (a<0.0 && b<0.0) {return (theta-Math.PI);}
    else if (a==0.0 && b>0.0) {return (Math.PI/2);}
    else if (a==0.0 && b<0.0){return (-Math.PI/2);}
    else {return (0.0);}
  }
  ...
  *@return z^(m/n).
  */
  public static Complex[] rationalpow(Complex z, int m, int n){
